...
### Addendum(사후 검증): 위 상관 가능성을 실제 수치로 확인함

PR 리뷰 과정에서 `data/raw/train.csv`로 직접 재계산해 위 문단의 "가능성"을
확정했다.

- `corr(passenger_fraction, total_burden) = 0.7665`,
  `corr(excl_passenger, total_burden) = 0.9999`(정의상 거의 항등) — 두 파생
  열 모두 byte-level 중복은 아니지만 **정보량 관점에서는 기존
  `sample__mutated_gene_count_log1p`의 노이즈 낀 재탕**에 가까웠다.
- 클래스별 `passenger_fraction` 평균은 원래 burden이 높은 클래스(SKCM
  0.238, LUSC 0.193, LUAD 0.162, STES 0.158)와 낮은 클래스(LAML 0.004,
  THCA 0.007, PCPG 0.010)로 그대로 갈렸다 — 이 파생 신호가 "암종 특이
  신호"가 아니라 "그 암종의 원래 burden 수준"을 다시 측정하고 있었다는
  뜻이다.
- 26개 클래스 평균의 표준편차(0.0642)가 클래스 내부 표준편차(0.1232)의
  절반 수준이다 — 클래스 간 차이보다 같은 클래스 안의 우연한 변동이 더
  커서, 통계적으로 약한 신호였다.
- LUSC(0.193)/STES(0.158) 평균 차이도 각 클래스 내부 표준편차(~0.14~0.16)에
  묻힐 정도로 작아, 위 표의 결과(LUSC 악화·STES 미미한 개선)와 일치한다.

**결론**: "장유전자 passenger burden을 요약해서 추가하면 도움이 될 것"이라는
가설은 이 검증으로 반증됐다 — 15개 유전자 중 어떤 부분집합을 골라 합산
count/fraction으로 압축하더라도, 그 압축값은 이미 존재하는 total burden
feature의 변형에 가까워질 수밖에 없는 구조다(passenger 유전자가 길어서
burden이 높을수록 더 자주 걸린다는 원래 가설 자체가, 파생 압축값을 burden의
대리 지표로 만드는 원인이기도 하다). **이 방향(passenger 유전자 그룹의
합산형 burden 파생 피처)은 추가 반복 없이 종결한다.** 개별 유전자 가중치
조정이나 다른 모델 구조를 쓰는 시도는 이 결론과 무관한 별도 가설이므로
새로 검증이 필요하다.
