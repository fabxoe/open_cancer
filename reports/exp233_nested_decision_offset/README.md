...
방향으로 과적합**하고 있다는 뜻입니다 — DLBC는 outer-train 표본이
30~31건뿐이고, 이를 inner 3-fold로 나누면 **inner fold당 평균 10건**
수준까지 줄어듭니다(다음으로 작은 클래스 ACC조차 outer-train 57건,
inner fold당 19건으로 거의 2배입니다). 이 정도 표본으로는 inner-CV로도
안정적인 offset 신호를 얻기엔 근본적으로 부족합니다. 이는 이번 세션에서
반복 확인한 DLBC의 구조적 config-민감성(seed·
...
## 부가 발견: DLBC를 빼면 5개 fold 전부 일관되게 개선

기각 판정과는 별개로, DLBC를 제외한 25개 클래스만으로 fold별 Macro F1을
다시 계산하면 결과가 완전히 달라집니다.

| outer fold | before(25클래스) | after(25클래스) | delta(25클래스) | delta(26클래스, 참고) |
|---:|---:|---:|---:|---:|
| 0 | 0.4234519289 | 0.4249350000 | +0.0014831 | +0.0002605 |
| 1 | 0.4186772515 | 0.4260924575 | +0.0074152 | +0.0015356 |
| 2 | 0.4060355485 | 0.4125283098 | **+0.0064928** | -0.0011613 |
| 3 | 0.4259492801 | 0.4349512610 | **+0.0090020** | -0.0031786 |
| 4 | 0.4337836619 | 0.4364230759 | +0.0026394 | +0.0025379 |

**5개 fold 전부 개선**합니다 — 26클래스 기준으로 하락했던 fold 2·3도
DLBC를 빼면 오히려 가장 크게 개선되는 fold로 바뀝니다(fold 2의 DLBC
offset +0.8, fold 3의 -0.7이 각각 강한 부스트/억제라 다른 25개 클래스의
진짜 개선을 덮어버린 것). 전체 OOF Macro F1(25클래스)은
0.4219785747 → 0.4289536818, **delta +0.0069751072**로 26클래스 기준
delta(+0.0019573460)의 **약 3.5배**입니다.

이는 offset 방법론 자체가 무효하다는 뜻이 아니라, **실패가 DLBC(그리고
아마 비슷한 극소수 클래스)에 국한**돼 있고 표본이 충분한 클래스에서는
일관되게 잘 작동한다는 뜻입니다. 표본 수 기준으로 극소수 클래스를 offset
탐색에서 제외하는 후속 실험을
[#276](https://github.com/fabxoe/open_cancer/issues/276)에서 진행한다.
