...
## 예비 관찰: feature-독립 노이즈 바닥 (n=2, 결론 보류)
...
두 seed 간 delta 상관은 0.21(낮음, 무작위성과 일치). feature 추가 3건의
표준편차(0.054~0.064)와 비교하면 1.3~1.9배 차이가 있지만, **n=2로는
이 차이가 실재하는 신호인지 우연인지 판단할 수 없다** — 0.033과
0.043 두 값 자체가 30% 차이가 나는 상황에서, seed를 더 늘리면 이
범위가 넓어질 수도(0.02~0.06대) 좁아질 수도(feature 효과 std가 확실히
더 크다는 쪽으로) 있다.

**결론을 내리지 않는다.** 추가 seed(2001/2002/2003) 확보 후
[#251](https://github.com/fabxoe/open_cancer/issues/251)에서 5개
seed의 delta std 분포를 정리하고, feature 추가 3건이 그 분포에서
어느 위치인지 판단한다. 이 예비 관찰이 만약 굳어지면(즉 feature 효과가
노이즈 바닥과 통계적으로 구분되지 않으면), 팀의 "소수 클래스 F1 악화
없음" promotion 게이트 자체에 노이즈 마진을 반영해야 하는지를 별도
팀 공유용 Task Issue로 다시 제기할 사안이다 — 이 문서 하나로 끝낼
범위가 아니므로 지금은 승격하지 않는다.
