...
4. **propensity 기반 test-like 서브셋 사전 검증의 실효성 입증**(#450, #457, #464) — EXP-253이 Local 게이트를 통과하고도 Public에서 -0.0178 무너졌던 실패를, 같은 조합(XGBoost+LightGBM)의 고정 블렌드(#450)·비율 스윕(#464)·stacking(#457) 세 가지 앙상블 기법 전부 Public 제출 없이 로컬 test-like 서브셋 체크만으로 사전 차단. #464는 특히 "가중치를 조정하면 살릴 수 있지 않을까"라는 자연스러운 다음 질문에 4개 비율(0.9/0.1~0.6/0.4) 전부 실패로 답해, 이 실패가 하이퍼파라미터 튜닝으로 해결될 문제가 아니라 이 두 base 모델 조합 자체의 구조적 한계임을 확정했다. 앞으로 모델 다양성/블렌드/스태킹 시도에 재사용 가능한 완료 조건 템플릿(`scripts/check_exp450_test_like_subset.py`, `scripts/check_exp457_test_like_subset.py`, `scripts/check_exp464_test_like_subset.py`).

## 재현성 상태

- #295/#296/#367/#421/#440/#449/#450/#457/#464: 전부 스크립트+원본 데이터 커밋 완료, 재실행 가능. #440/#449/#450/#457/#464는 INFERENCE_VERIFIED까지 자동 완료.
