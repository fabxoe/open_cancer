...
정확한 패키지 버전을 설치합니다. Jupyter와 pytest도 일반 의존성에 포함되어 별도
그룹 옵션이 필요 없습니다. `--frozen`은 초기 설정 중 팀 공통 lock 파일을 임의로
바꾸지 않게 합니다.
...
```text
Issue #12 작업이야.
목표: Issue 번호에서 자동 파생되는 EXP-012 XGBoost baseline을 공용 5-fold
split으로 구현해줘.
...
`data/README.md`의 SHA-256 및 Git에서 받은 원본과 비교합니다. 원본 데이터를 직접
편집하지 않습니다.
