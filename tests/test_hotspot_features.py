...
def test_build_hotspot_augmented_features_accepts_residue_position_options(
    tmp_path: Path,
) -> None:
    train = tmp_path / "train.csv"
    test = tmp_path / "test.csv"
    ...
    assert "BRAF__max_residue_position" in names
    assert train_matrix[0, names.index("BRAF__max_residue_position")] == 600
    assert names[-len(HOTSPOT_FEATURE_NAMES) :] == list(HOTSPOT_FEATURE_NAMES)
