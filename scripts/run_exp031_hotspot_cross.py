#!/usr/bin/env python
"""Run EXP-031 attempt 3: EXP-005 features + known cancer hotspot position features.

Attempts 1-2 re-aggregated information already present at the gene level
(COSMIC protect-gene x mutation-type counts) and both underperformed EXP-005.
This attempt targets genuinely new information instead: whether a mutation
lands on a specific, well-established codon (e.g. BRAF V600, PIK3CA H1047)
rather than merely "somewhere in the gene". The hotspot table is restricted
to positions verified internally consistent in this dataset
...
CONFIG_PATH = ROOT / "configs" / "exp031_hotspot_cross.yaml"
TRAIN_PATH = ROOT / "data" / "raw" / "train.csv"
TEST_PATH = ROOT / "data" / "raw" / "test.csv"
SAMPLE_SUBMISSION_PATH = ROOT / "data" / "raw" / "sample_submission.csv"
FEATURE_DIR = ROOT / "data" / "processed" / "hotspot_cross_features"
...
def main() -> None:
    ...
    artifact_slug = "exp031_hotspot_cross"
    ...
    feature_report = build_hotspot_augmented_features(TRAIN_PATH, TEST_PATH, FEATURE_DIR)
    ...
    resolved_config = {
        ...
        "training": {
            **config["training"],
            ...
            "command": "uv run python scripts/run_exp031_hotspot_cross.py",
        },
        "environment": {
            "python": sys.version,
    ...
    metrics = {
        ...
        "notes": (
            "EXP-005 gene x mutation-type sparse features (30,697) + 19 known "
            "cancer hotspot position indicators + 1 total hotspot count "
            "(BRAF/CTNNB1/EGFR/GNAS/HRAS/IDH1/IDH2/PIK3CA/TP53), verified "
            "internally consistent via scripts/explore_hotspot_numbering_consistency.py. "
            "KRAS/NRAS hotspots omitted: those genes are absent from this panel."
        ),
    }
