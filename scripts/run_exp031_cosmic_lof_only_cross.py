#!/usr/bin/env python
"""Run EXP-031 attempt 2: EXP-005 features + COSMIC protect-gene LOF count only.

Attempt 1 (`run_exp031_cosmic_mutation_type_cross.py`) added 8 protect-gene
cross features and scored slightly below EXP-005 (0.395607 vs 0.404380 OOF
Macro F1). This attempt keeps only the single feature the experiment was
proposed around (loss-of-function count among COSMIC protect genes) to check
whether the other 7 counts were adding noise rather than signal.
"""
...
CONFIG_PATH = ROOT / "configs" / "exp031_cosmic_lof_only_cross.yaml"
TRAIN_PATH = ROOT / "data" / "raw" / "train.csv"
TEST_PATH = ROOT / "data" / "raw" / "test.csv"
SAMPLE_SUBMISSION_PATH = ROOT / "data" / "raw" / "sample_submission.csv"
FEATURE_DIR = ROOT / "data" / "processed" / "cosmic_lof_only_cross_features"
...
def main() -> None:
    ...
    feature_report = build_cosmic_mutation_features(
        TRAIN_PATH, TEST_PATH, protect_genes_path, FEATURE_DIR, cross_features
    )
    ...
    resolved_config = {
        ...
        "training": {
            **config["training"],
            ...
            "command": "uv run python scripts/run_exp031_cosmic_lof_only_cross.py",
        },
        "environment": {
            "python": sys.version,
    ...
    metrics = {
        ...
        "notes": (
            "EXP-005 gene x mutation-type sparse features (30,697) + 1 COSMIC "
            "protect-gene cross feature (cosmic__protect_lof_count: nonsense + "
            "frameshift count restricted to the EXP-012 protect gene "
            "whitelist). Attempt 2 after attempt 1's 8-feature cross "
            "underperformed EXP-005; isolates the single LOF-focused feature."
        ),
    }
