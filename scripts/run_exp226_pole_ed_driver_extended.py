#!/usr/bin/env python
"""Run EXP-226: EXP-094 Feature Spec v1 + POLE_ED_driver_extended.

Confirmatory follow-up to EXP-181 (D, rejected). D's 4-seed stability
check found COAD's F1 delta positive across all 4 seeds while UCEC/DLBC
were not direction-consistent. This experiment checks whether that COAD
direction reproduces at a slightly larger sample (22 -> 28 positive rows,
fold-3 concentration eased from 1 to 2). Single official seed (42) only --
no multi-seed stability check, since this is a confirmation of an already
seed-checked pattern, not a new stability claim. Gate pass/fail is
recorded but is not the deciding factor; see Issue #226 for scope.
"""
...
from open_cancer.pole_ed_features import pole_ed_driver_extended_family
...
CONFIG = ROOT / "configs" / "exp226_pole_ed_driver_extended.yaml"
...
def build_resolved_config(
    *,
    ...
    return {
        ...
        "training": {"balanced_sample_weight": True, "official_model_seed_base": 42},
        "environment": {
            "python": sys.version,
...
def main() -> None:
    ...
    if context.experiment_id != "EXP-226" or dirty:
        raise RuntimeError(
            "EXP-226은 clean issue-226 브랜치에서만 실행해야 합니다.\n" + dirty
        )
    ...
    fitted_family = pole_ed_driver_extended_family().fit(train_raw)
    ...
    coad_direction_matches_d = watch_class_deltas.get("COAD", 0.0) > 0

    metrics = {
        "experiment_id": "EXP-226",
        ...
        "notes": (
            "Confirmatory follow-up to EXP-181 (D, rejected): single official "
            "seed 42 only, no multi-seed stability check (see Issue #226 "
            "scope). EXP-094 frozen Feature Spec v1 + pole__ed_driver_extended "
            "(21 ED driver substitutions, Feature Factory family, no "
            "file-backed KnowledgeProvenance). "
            f"Train positive rate {positive_rate_train:.6f} (28/6201), test "
            f"positive rate {positive_rate_test:.6f}. Fold positive counts: "
            f"{fold_positive_counts} (D had {{0:8,1:5,2:5,3:1,4:3}}, fold 3 "
            f"eased from 1 to 2). Watch-class F1 deltas: {watch_class_deltas}. "
            f"COAD direction matches D's positive delta: {coad_direction_matches_d}. "
            ...
            f"Gate verdict (recorded, not decisive per scope): "
        ...
        json.dumps(
            {
                **verdict,
                ...
                "coad_direction_matches_d": coad_direction_matches_d,
                "d_coad_delta_for_reference": 0.005080180477105012,
