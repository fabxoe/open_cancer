...
def main() -> None:
    ...
    train_flag = compute_any_nonsilent_flag(train_raw, CELL_CYCLE_GENES).astype(np.float32)
    test_flag = compute_any_nonsilent_flag(test_raw, CELL_CYCLE_GENES).astype(np.float32)
    ...
    metrics = {
        ...
        "notes": (
            "EXP-094 frozen Feature Spec v1 + P_any_nonsilent_cellcycle "
            "(Cell Cycle pathway, 15 genes, Issue #167 catalog, zero overlap "
            "with EXTENDED_HOTSPOTS). "
