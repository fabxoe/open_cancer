#!/usr/bin/env python
"""Run EXP-031: EXP-005 mutation-type features + COSMIC protect-gene cross features."""
...
CONFIG_PATH = ROOT / "configs" / "exp031_cosmic_mutation_type_cross.yaml"
TRAIN_PATH = ROOT / "data" / "raw" / "train.csv"
TEST_PATH = ROOT / "data" / "raw" / "test.csv"
SAMPLE_SUBMISSION_PATH = ROOT / "data" / "raw" / "sample_submission.csv"
FEATURE_DIR = ROOT / "data" / "processed" / "cosmic_mutation_type_cross_features"
...
def main() -> None:
    ...
    artifact_slug = "exp031_cosmic_mutation_type_cross"
    ...
    resolved_config = {
        ...
        "training": {
            **config["training"],
            ...
            "command": "uv run python scripts/run_exp031_cosmic_mutation_type_cross.py",
        },
        "environment": {
            "python": sys.version,
    ...
    metrics = {
        ...
        "notes": (
            "EXP-005 gene x mutation-type sparse features (30,697) + 8 COSMIC "
            "protect-gene cross features (per-type and LOF counts restricted to "
            "the EXP-012 protect gene whitelist). Whitelist license status "
            "unresolved, so it is not committed; see feature contract for its "
            "SHA-256 in the resolved config."
        ),
    }
