#!/usr/bin/env python
"""EXP-021 attempt 3: full mutation-presence features + one COSMIC-weighted
burden column, XGBoost.

Keeps all 4,384 mutation-presence features (like EXP-003) and appends a
single engineered column: a per-sample weighted sum over the protect genes
(weight 1.0) plus each fold's top correlated genes (weight = that fold's
|correlation| with protect burden, from EXP-021 attempt 2). The correlated
genes and their weights are recomputed independently inside every fold using
only that fold's training rows, so no validation-fold information leaks into
the derived feature (see PROJECT_CONTEXT.md 5절: feature selection은 fold
train에서만 fit). Unlike attempt 2, no original feature is dropped here —
this tests whether the aggregate signal helps on top of the full feature set
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/exp021_cosmic_weighted_burden_baseline.yaml"),
...
def main() -> None:
    ...
    for fold in range(n_splits):
        ...
        train_burden = weighted_protect_burden(
            x_train_full[train_indices], gene_columns, protect_genes_in_columns, weights
        )
        valid_burden = weighted_protect_burden(
            x_train_full[valid_indices], gene_columns, protect_genes_in_columns, weights
        )
        test_burden = weighted_protect_burden(
            x_test_full, gene_columns, protect_genes_in_columns, weights
        )

        x_fold_train = sparse.hstack(
            [x_train_full[train_indices], train_burden.reshape(-1, 1)],
            format="csr",
            dtype=np.float32,
        )
        x_fold_valid = sparse.hstack(
            [x_train_full[valid_indices], valid_burden.reshape(-1, 1)],
            format="csr",
            dtype=np.float32,
        )
        x_fold_test = sparse.hstack(
            [x_test_full, test_burden.reshape(-1, 1)],
            format="csr",
            dtype=np.float32,
        )
        ...
        print(
            ...
            f"(4384 + 1 weighted_burden, correlated={len(weights)}) "
    ...
    metrics: dict[str, Any] = {
        ...
        "notes": (
            ...
            f"burden 파생 컬럼 1개(protect {len(protect_genes_in_columns)}개 "
            f"가중치 1.0, fold별 상관 상위 {top_k}개는 |correlation| 가중치). "
            "가중치는 fold train에서만 재계산(leakage 방지). 화이트리스트/가중치 "
            "상세는 라이선스 미확정으로 Git 미포함, config.resolved.yaml의 "
            "feature_manifest.weighted_burden 참고."
        ),
    }
    ...
    resolved_config: dict[str, Any] = {
        **config,
        ...
        "feature_manifest": {
            ...
            "final_feature_count": int(x_train_full.shape[1] + 1),
            "class_order": list(CLASS_LABELS),
            "weighted_burden": {
                "method": (
                    "protect_genes(weight=1.0) + per_fold_top_correlated"
                    "(weight=|correlation|), summed per row into 1 new column"
                ),
        ...
        "command": {
            "train": (
                ...
                f"scripts/run_exp021_cosmic_weighted_burden_baseline.py "
