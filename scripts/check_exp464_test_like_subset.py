#!/usr/bin/env python
"""Mandatory completion-condition check for EXP-464 (Issue #464).

Compares the official EXP-464 arm (0.7/0.3 EXP-374/EXP-449 blend, the
least-bad point from the 4-ratio sweep) against EXP-374 alone on the
train_domain_propensity.csv (#292) top-quartile "test-like" subset --
same boundary as EXP-351/EXP-450/EXP-457, read-only diagnostic use only.
"""
...
BLEND_OOF_PATH = ROOT / "oof" / "exp464_blend_ratio_sweep.csv"
...
OUT_PATH = ROOT / "reports" / "exp464_blend_ratio_sweep" / "test_like_subset_check.json"
...
def main() -> None:
    ...
    result = {
        ...
        "issue": 464,
        "weights": {"EXP-374": 0.7, "EXP-449": 0.3},
