#!/usr/bin/env python
"""Run EXP-296: EXP-094 Feature Spec v1 + CTNNB1 D32 + CTNNB1 S33.

Position-level hotspot pilot (Issue #296), same pattern as the POLE ED
hotspot pilots (EXP-181/226): a single-gene, position-specific refinement
of an already-adopted hotspot family. D32/S33 are two of the four residues
of the beta-catenin N-terminal phosphodegron motif shared with the existing
CTNNB1 S37/S45 columns (`hotspot_features.EXTENDED_HOTSPOTS`). Precheck
(Issue #296 discussion) confirmed: Vera Health gates A/B/C pass for both
positions across all 5 folds, no mutation-burden confound (S33 carriers in
UCEC actually have *lower* burden than non-carriers, consistent with the
known low-burden CTNNB1-mutant UCEC molecular subtype), and zero sample-ID
overlap between D32/S33/S37/S45 -- so both are added as two separate
columns, not merged (avoiding the EXP-058 APC/CTNNB1 "combined position ->
information loss" pattern).

A `find_semantically_equivalent_features` check against the frozen v1 base
(which already contains hotspot__CTNNB1_37/45) is run before training to
formally confirm neither new column duplicates an existing one.
"""
...
from open_cancer.ctnnb1_hotspot_features import ctnnb1_d32_family, ctnnb1_s33_family
...
def build_resolved_config(
    *,
    ...
    semantic_equivalence_matches: dict[str, str],
    ...
    return {
        ...
        "ctnnb1_family": family_registry,
        "semantic_equivalence_check": {
            "reference": "frozen Feature Spec v1 (includes hotspot__CTNNB1_37/45)",
            "matches": semantic_equivalence_matches,
            "note": (
                "empty dict means neither hotspot__CTNNB1_32 nor "
                "hotspot__CTNNB1_33 is byte-identical to any existing v1 "
                "column on train rows"
            ),
        },
...
def main() -> None:
    ...
    if context.experiment_id != "EXP-296" or dirty:
        raise RuntimeError(
            "EXP-296은 clean issue-296 브랜치에서만 실행해야 합니다.\n" + dirty
        )
    ...
    fitted_d32 = ctnnb1_d32_family().fit(train_raw)
    fitted_s33 = ctnnb1_s33_family().fit(train_raw)
    train_d32 = transform_checked(fitted_d32, train_raw)
    train_s33 = transform_checked(fitted_s33, train_raw)
    test_d32 = transform_checked(fitted_d32, test_raw)
    test_s33 = transform_checked(fitted_s33, test_raw)
    train_flag_matrix = sparse.hstack([train_d32, train_s33], format="csr")
    test_flag_matrix = sparse.hstack([test_d32, test_s33], format="csr")
    flag_feature_names = (*fitted_d32.descriptor.feature_names, *fitted_s33.descriptor.feature_names)
    family_registry = build_family_registry([fitted_d32, fitted_s33])

    semantic_equivalence_matches = find_semantically_equivalent_features(
        train_flag_matrix, flag_feature_names, x_train_base, base_feature_names
    )
    if semantic_equivalence_matches:
        raise RuntimeError(
            "CTNNB1 D32/S33가 기존 Feature Spec v1 열과 중복됩니다: "
            + json.dumps(semantic_equivalence_matches, ensure_ascii=False)
        )

    positive_rate_train = {
        "hotspot__CTNNB1_32": float(train_d32.toarray().mean()),
        "hotspot__CTNNB1_33": float(train_s33.toarray().mean()),
    }
    positive_rate_test = {
        "hotspot__CTNNB1_32": float(test_d32.toarray().mean()),
        "hotspot__CTNNB1_33": float(test_s33.toarray().mean()),
    }

    watch_classes = config.get("watch_classes", [])
    d32_flag = train_d32.toarray().ravel()
    s33_flag = train_s33.toarray().ravel()
    watch_positive_rate_by_class = {
        cls: {
            "hotspot__CTNNB1_32": float(d32_flag[train_raw["SUBCLASS"] == cls].mean()),
            "hotspot__CTNNB1_33": float(s33_flag[train_raw["SUBCLASS"] == cls].mean()),
        }
    ...
    fold_positive_counts = {
        "hotspot__CTNNB1_32": {int(fold): int(d32_flag[folds == fold].sum()) for fold in range(5)},
        "hotspot__CTNNB1_33": {int(fold): int(s33_flag[folds == fold].sum()) for fold in range(5)},
    }
    ...
    resolved_config = build_resolved_config(
        ...
        semantic_equivalence_matches=semantic_equivalence_matches,
    ...
    metrics = {
        ...
        "notes": (
            "EXP-094 frozen Feature Spec v1 + hotspot__CTNNB1_32 + "
            "hotspot__CTNNB1_33, registered as two Feature Factory families "
            "(Ctnnb1Family; no file-backed KnowledgeProvenance, hardcoded "
            "literature-cited literal, see module docstring). Semantic "
            "equivalence check against v1 (which already has "
            "hotspot__CTNNB1_37/45) found no duplicate columns. "
            f"Train positive rates {positive_rate_train}, test positive "
            f"rates {positive_rate_test}. Fold positive counts: "
            f"{fold_positive_counts}. Watch-class (UCEC/LIHC) F1 deltas: "
            f"{watch_class_deltas}. "
