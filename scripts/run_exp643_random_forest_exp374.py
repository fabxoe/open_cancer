#!/usr/bin/env python
"""Run EXP-643: RandomForest on EXP-374's exact feature set (legacy model diversity).
...
directly, the same pattern EXP-449 (LightGBM) and EXP-459 (CatBoost) used.
The sole change is the model: RandomForestAdapter with EXP-596's exact
hyperparameters (already proven at a comparable ~35,119-dim sparse matrix on
this same hardware, 5-fold total 62s), so no preflight timing check is
needed here.
"""
...
from open_cancer.model_runner import RandomForestAdapter
...
CONFIG_PATH = ROOT / "configs" / "exp643_random_forest_exp374.yaml"
...
SLUG = "exp643_random_forest_exp374"
...
def main() -> None:
    ...
    if context.experiment_id != "EXP-643" or dirty:
        raise RuntimeError("EXP-643은 clean issue-643 브랜치에서만 실행해야 합니다.\n" + dirty)
    ...
    for fold in range(n_splits):
        ...
        adapter = RandomForestAdapter(dict(model_params), seed=seed + fold)
        ...
        adapter.save(model_dir / f"fold_{fold:02d}{adapter.file_suffix}")
    ...
    resolved_config = {
        ...
        "model": {"class": "sklearn.ensemble.RandomForestClassifier", "parameters": model_params},
        "training": {
            **config["training"],
            "command": "uv run python scripts/run_exp643_random_forest_exp374.py",
        },
        "environment": {
            "python": sys.version,
    ...
    write_model_run_records(
        ...
        artifacts={
            **{f"checkpoint_fold_{fold}": model_dir / f"fold_{fold:02d}{RandomForestAdapter.file_suffix}" for fold in range(n_splits)},
    ...
    # Inference-only reproducibility check: reload saved RandomForest models,
    ...
    for fold in range(n_splits):
        ...
        x_test_fold = sparse.hstack([x_test_dropped, extra.test], format="csr", dtype=np.float32)
        model = joblib.load(model_dir / f"fold_{fold:02d}{RandomForestAdapter.file_suffix}")
        raw = model.predict_proba(x_test_fold)
        fold_pred = np.zeros((x_test_fold.shape[0], len(CLASS_LABELS)), dtype=np.float64)
        fold_pred[:, model.classes_.astype(int)] = raw
