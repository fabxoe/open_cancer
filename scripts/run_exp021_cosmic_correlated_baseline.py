#!/usr/bin/env python
"""EXP-021 attempt 2: protect genes + per-fold correlated genes, XGBoost.

Extends the EXP-021 hypothesis (COSMIC CGC v104 "protect" gene feature
selection) by adding, for each fold, the non-protect genes whose mutation
indicator correlates most strongly with the protect-gene mutation-burden
score. The correlation is recomputed independently inside every fold using
only that fold's training rows, so no validation-fold information leaks into
the feature selection (see PROJECT_CONTEXT.md 5절: feature selection은 fold
train에서만 fit).
"""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/exp021_cosmic_correlated_baseline.yaml"),
...
def main() -> None:
    ...
    fold_gene_selection: list[dict[str, Any]] = []
    ...
    for fold in range(n_splits):
        ...
        correlated_genes = select_correlated_genes(
            fold_train_matrix, gene_columns, protect_genes_in_columns, top_k
        )
        fold_gene_set = protect_genes_in_columns | set(correlated_genes)
        fold_gene_positions = [
            i for i, gene in enumerate(gene_columns) if gene in fold_gene_set
        ]
        fold_gene_names = [gene_columns[i] for i in fold_gene_positions]
        fold_gene_selection.append(
            {
                "fold": fold,
                "protect_gene_count": len(protect_genes_in_columns),
                "correlated_gene_count": len(correlated_genes),
                "total_gene_count": len(fold_gene_names),
                "gene_order_sha256": sha256_lines(fold_gene_names),
            }
        )

        x_fold_train = x_train_full[train_indices][:, fold_gene_positions]
        x_fold_valid = x_train_full[valid_indices][:, fold_gene_positions]
        x_fold_test = x_test_full[:, fold_gene_positions]
        ...
        print(
            f"fold={fold} genes={len(fold_gene_names)} "
            f"(protect={len(protect_genes_in_columns)}+correlated={len(correlated_genes)}) "
    ...
    metrics: dict[str, Any] = {
        ...
        "notes": (
            "COSMIC protect 유전자 + fold별 재계산한 protect-burden 상관 상위 "
            f"{top_k}개 확장. fold 평균 피처 수={avg_total_genes:.1f} "
            f"(protect {len(protect_genes_in_columns)}개 고정 + fold별 correlated "
            f"{top_k}개). 화이트리스트/피처 목록은 라이선스 미확정으로 Git 미포함, "
            "config.resolved.yaml의 feature_manifest.gene_selection 참고."
        ),
    }
    ...
    resolved_config: dict[str, Any] = {
        **config,
        ...
        "feature_manifest": {
            ...
            "gene_selection": {
                "method": "protect_genes + per_fold_top_correlated_with_protect_burden",
        ...
        "command": {
            "train": (
                ...
                f"scripts/run_exp021_cosmic_correlated_baseline.py "
        ...
        json.dumps(
            {
                ...
                "avg_total_genes": avg_total_genes,
                "submission": _relative(submission_path),
                "submission_sha256": submission_summary["sha256"],
                "metrics": _relative(metrics_path),
