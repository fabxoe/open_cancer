#!/usr/bin/env python
"""Run EXP-550 hierarchical TF-IDF multinomial Logistic Regression."""
...
CONFIG_PATH = ROOT / "configs/exp550_hierarchical_tfidf_logistic.yaml"
SLUG = "exp550_hierarchical_tfidf_logistic"
...
def main() -> None:
    ...
    if config["experiment_id"] != "EXP-550" or config["issue_number"] != 550:
        raise ValueError("EXP-550 identity mismatch")
    ...
    for fold in range(5):
        ...
        fold_test_probability = model.predict_proba(x_test).astype(np.float32)
        oof_probability[valid_idx] = valid_probability
        test_probability += fold_test_probability / 5.0
        prediction = model.classes_[np.argmax(valid_probability, axis=1)]
        fold_metrics.append(
            {
                "fold": fold,
                "macro_f1": float(
                    f1_score(
                        y[valid_idx], prediction, average="macro", zero_division=0
                    )
                ),
                "accuracy": float(accuracy_score(y[valid_idx], prediction)),
                "log_loss": float(
                    log_loss(
                        y[valid_idx], valid_probability, labels=CLASS_LABELS
                    )
                ),
                "best_iteration": None,
        ...
        print(
            f"fold {fold}: {fold_metrics[-1]['macro_f1']:.10f} "
            f"logloss={fold_metrics[-1]['log_loss']:.10f} "
            f"({time.perf_counter() - fold_started:.1f}s)",
            flush=True,
        )

    oof_prediction = np.asarray(CLASS_LABELS)[np.argmax(oof_probability, axis=1)]
    test_prediction = np.asarray(CLASS_LABELS)[np.argmax(test_probability, axis=1)]
    ...
    per_class = f1_score(
        y, oof_prediction, labels=CLASS_LABELS, average=None, zero_division=0
    )
    finished = datetime.now(timezone.utc)
    metrics = {
        "experiment_id": "EXP-550",
        ...
        "oof": {
            "macro_f1": float(
                f1_score(y, oof_prediction, average="macro", zero_division=0)
            ),
            ...
            "per_class_f1": dict(
                zip(CLASS_LABELS, per_class.tolist(), strict=True)
            ),
            "confusion_matrix": confusion_matrix(
                y, oof_prediction, labels=CLASS_LABELS
            ).tolist(),
        },
        "leaderboard": None,
        "runtime": {
        ...
        "decision": "PENDING_PROBABILITY_MODEL_GATE",
        "artifacts": {
    ...
    (report_dir / "README.md").write_text(
        "\n".join(
            [
                ...
                f"- predict_proba Log Loss: `{metrics['oof']['log_loss']:.10f}`",
