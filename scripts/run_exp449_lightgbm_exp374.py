#!/usr/bin/env python
"""Run EXP-449: LightGBM on EXP-374's exact feature set (model diversity).
...
directly. The sole change is the model: LightGBM with EXP-209's fixed
hyperparameters (no retuning).

Not run via run_hotspot_xgb.main() (XGBoost-only); replicates its base
feature build + fold loop, swapping in LightGBMAdapter.
"""
...
CONFIG_PATH = ROOT / "configs" / "exp449_lightgbm_exp374.yaml"
...
def main() -> None:
    ...
    for fold in range(n_splits):
        ...
        adapter = LightGBMAdapter(dict(model_params), seed=seed + fold)
        ...
        adapter.save(model_dir / f"fold_{fold:02d}.txt")
    ...
    # Inference-only reproducibility check: reload saved LightGBM boosters,
    # re-predict on test, confirm byte-identical submission and near-zero
    # probability drift (deterministic: true, force_col_wise: true).
    ...
    for fold in range(n_splits):
        ...
        fold_pred = booster.predict(x_test_fold)
