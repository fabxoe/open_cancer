#!/usr/bin/env python
"""Mandatory completion-condition check for EXP-465 (Issue #465).

Compares the EXP-465 feature-subset blend (hotspot-only + sample-
aggregate-burden-only XGBoost, 0.5/0.5) against its EXP-374 parent on the
train_domain_propensity.csv (#292) top-quartile "test-like" subset --
same boundary as EXP-351/EXP-450/EXP-457/EXP-464, read-only diagnostic
use only.
"""
...
BLEND_OOF_PATH = ROOT / "oof" / "exp465_feature_subset_ensemble.csv"
...
OUT_PATH = ROOT / "reports" / "exp465_feature_subset_ensemble" / "test_like_subset_check.json"
