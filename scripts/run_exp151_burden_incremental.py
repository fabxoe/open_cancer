#!/usr/bin/env python
"""Run EXP-158: EXP-094 frozen features plus missense burden."""
...
CONFIG = ROOT / "configs" / "exp158_missense_burden.yaml"
...
def main() -> None:
    ...
    if context.experiment_id != "EXP-158" or dirty:
        raise RuntimeError("EXP-158는 clean issue-158 브랜치에서만 실행해야 합니다.")
    slug = "exp158_missense_burden"
    ...
    train_burden = np.log1p(build_summary(TRAIN)["missense_count"].to_numpy(dtype=np.float32))
    test_burden = np.log1p(build_summary(TEST, with_label=False)["missense_count"].to_numpy(dtype=np.float32))
    ...
    metrics = {
        "experiment_id": "EXP-158", "record_role": "official", "status": "COMPLETED",
        "owner": git("config", "user.name") or "unknown", "issue_number": 158,
        ...
        "notes": "EXP-094 frozen Feature Spec + log1p(missense_count); test labels unused.",
    }
