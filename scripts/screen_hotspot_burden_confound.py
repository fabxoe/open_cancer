...
RUN_MODE=explore. No EXP-ID, no model training. This is the standard
methodology addition adopted from Issue #295: a DominoEffect-style
recurrence/window screening (e.g. `dominoeffect_screening.py`-style output)
can surface "hotspots" that are really an artifact of a handful of
hypermutated samples in a naturally low-TMB (or hypermutator-subtype-prone)
cancer type recurring across many unrelated gene positions -- discovered via
a 6-gene ACC cluster (LRIG1 x2, SOWAHC, CMPK2, PEX6, NFKB2, TM7SF2) that all
turned out to share carriers from a set of high-burden ACC samples.
...
Neither check proves a candidate is *not* real biology (e.g. a UCEC POLE/MSI
hypermutator subtype driving broad recurrence is itself a known real
phenomenon, not "corrupted data") -- they identify candidates whose
recurrence signal in this panel cannot be trusted at face value without
independent literature support and are not safe to promote to a hotspot
feature straight from the screening output.
"""
...
def screen_burden_confound(
    ...
    `candidates` must have columns: gene, position, reference_aa (as produced
    by `dominoeffect_screening.py`'s `..._all_candidates.csv` output).
    ...
    result_df["burden_flag"] = pd.cut(
        ...
        bins=[-float("inf"), BURDEN_MILD_CONCERN_RATIO, BURDEN_ARTIFACT_RATIO, float("inf")],
        labels=["clean", "mild_concern", "artifact_suspect"],
    )
...
def recommended_exclusions(result_df: pd.DataFrame, cluster_df: pd.DataFrame) -> set[str]:
    ...
    reliable_suspect = result_df[
        (result_df["burden_flag"] == "artifact_suspect") & result_df["burden_reliable"]
    ]
    reliable_keys = set(reliable_suspect["gene"] + "_" + reliable_suspect["position"].astype(str))
...
if __name__ == "__main__":
    ...
    print(f"후보 {len(candidates_table)}건 중 {len(exclusions)}건 제외 권고")
