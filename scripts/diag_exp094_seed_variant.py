...
official exp094 paths. OOF outputs go under the Git-excluded
`oof/analysis/issue251_dlbc_noise_floor/`; compact metadata remains under
`reports/analysis/dlbc_noise_floor_data/`.
...
META_DIR = ROOT / "reports" / "analysis" / "dlbc_noise_floor_data"
...
feature_dir = OOF_DIR / "_diag_exp094_v1_features"  # shared cache, read-only reuse across seeds
model_dir = OOF_DIR / f"_diag_exp094_seed{seed_base}_models"
...
oof_path = OOF_DIR / f"diag_exp094_seed{seed_base}_oof.csv"
oof_df.to_csv(oof_path, index=False)
print(f"Wrote {oof_path}")
print(f"Elapsed: {time.perf_counter() - clock:.1f}s")

with open(META_DIR / f"diag_exp094_seed{seed_base}_meta.json", "w", encoding="utf-8") as fh:
    json.dump(
        {
