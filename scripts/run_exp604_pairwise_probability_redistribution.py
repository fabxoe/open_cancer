#!/usr/bin/env python
"""Run EXP-604: pairwise-only probability mass redistribution for EXP-374.

Issue #604, follow-up to #233/#276/#515 (all rejected: every prior
mechanism applied `softmax(z)*exp(o)` then renormalized over all 26
classes, which coupled every class's probability mass into a shared
zero-sum competition -- #515 restricted the *eligible* classes to 4 but
still measured a 0.0994 absolute F1 delta across the 22 "untouched"
classes against a 0.01 gate, because renormalization itself disturbs
them regardless of which classes are searched).

This experiment replaces the mechanism itself
(`open_cancer.nested_decision_offset.apply_pairwise_redistribution`):
for each target pair (KIPAN/KIRC, GBMLGG/LGG), the pair's own
probability sub-budget `s = p_a + p_b` is held fixed and only the
internal split ratio is shifted in logit space. Every class outside the
two pairs is mathematically guaranteed untouched -- not just intended to
be untouched -- since their columns are never read or written and the
row sum is trivially conserved.
"""
...
CONFIG_PATH = ROOT / "configs" / "exp604_pairwise_probability_redistribution.yaml"
TRAIN_PATH = ROOT / "data" / "raw" / "train.csv"
TEST_PATH = ROOT / "data" / "raw" / "test.csv"
EXP374_CONFIG_PATH = ROOT / "configs" / "exp374_stop_isoform_residue_mask.yaml"
ISSUE = 604
EXP_ID = "EXP-604"
SLUG = "pairwise_probability_redistribution"
ARTIFACT_SLUG = f"exp604_{SLUG}"
PARENT_EXPERIMENT = "EXP-374"
RUNNER_COMMAND = "uv run python scripts/run_exp604_pairwise_probability_redistribution.py"
...
def main() -> None:
    ...
    if context.experiment_id != EXP_ID or dirty:
        raise RuntimeError(f"EXP-604는 clean issue-604 브랜치에서만 실행해야 합니다.\n{dirty}")
