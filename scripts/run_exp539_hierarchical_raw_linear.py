#!/usr/bin/env python
"""Run EXP-539 hierarchical raw-count LinearSVC canonical 5-fold."""
...
CONFIG_PATH = ROOT / "configs/exp539_hierarchical_raw_linear.yaml"
SLUG = "exp539_hierarchical_raw_linear"
...
def main() -> None:
    ...
    if config["experiment_id"] != "EXP-539" or config["issue_number"] != 539:
        raise ValueError("EXP-539 identity mismatch")
    ...
    for fold in range(5):
        ...
        joblib.dump(
            {"model": model, "adapter": adapter, "classes": CLASS_LABELS},
    ...
    metrics = {
        "experiment_id": "EXP-539", "record_role": "official", "status": "COMPLETED",
        "owner": "fabxoe", "issue_number": 539, "parent_experiment": None,
        ...
        "decision": "PENDING_COMPARISON_WITH_ROW_L2",
        "artifacts": {
    ...
    (report_dir / "README.md").write_text(
        "\n".join([
            "# EXP-539 hierarchical raw-count LinearSVC", "",
            "Parser-v4 canonical event의 hierarchical detail/global raw count 기준선입니다.", "",
            ...
            "다음 EXP에서는 vocabulary와 LinearSVC 설정을 유지하고 row-L2만 변경합니다.",
        ])+"\n", encoding="utf-8"
    )
