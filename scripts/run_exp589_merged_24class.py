#!/usr/bin/env python
"""Run EXP-589: merge KIRC into KIPAN and LGG into GBMLGG."""
...
CONFIG = ROOT / "configs" / "exp589_merged_24class.yaml"
TARGET_ALIASES = {"KIRC": "KIPAN", "LGG": "GBMLGG"}
MERGED_CLASS_LABELS = tuple(
    label for label in CLASS_LABELS if label not in TARGET_ALIASES
)


class Merged24ClassCosineFoldBuilder:
    """Rebuild EXP-527 class centroids in the same merged 24-class space."""
    ...
    def __call__(
        ...
        *,
        ...
        fitted_profile = ClassSemanticProfileFamily(
            MERGED_CLASS_LABELS, method="cosine"
        ).fit(self.train_semantic[train_indices], target)
        profile_train = fitted_profile.transform_train_leave_one_out(
        ...
        return FoldFeatureBundle(
            ...
            validation=sparse.hstack(
                [parent.validation, profile_valid], format="csr"
            ),
...
if __name__ == "__main__":
    main(
        CONFIG,
        fold_feature_builder=Merged24ClassCosineFoldBuilder(),
        ...
        runner_command="uv run python scripts/run_exp589_merged_24class.py",
        model_class_labels=MERGED_CLASS_LABELS,
        target_aliases=TARGET_ALIASES,
    )
