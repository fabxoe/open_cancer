...
- parser-v4 native 희소 행렬을 SAINT 입력용 128·256·512열로 제한할 때는
  validation/test 통계를 보지 않는 outer-train-only 의미 압축기를 사용한다.
  이름 없는 잠재벡터가 아니라 원본 gene-event 이름·family·순서를 보존하고,
  같은 fold의 작은 차원은 큰 차원의 prefix여야 한다. compatibility mutation/missing
  열을 native selector에 섞지 않는다. 구현·manifest·실행법은
  [`docs/SEMANTIC_COMPRESSION_SAINT_DATASET.md`](docs/SEMANTIC_COMPRESSION_SAINT_DATASET.md)를
  따른다.
