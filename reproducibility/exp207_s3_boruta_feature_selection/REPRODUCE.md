...
```bash
uv sync --frozen
git switch issue-207-s3-boruta-feature-selection
uv run python scripts/run_exp207_s3_boruta_feature_selection.py
```
...
이 기록은 `MANIFEST_COMPLETE`다. 독립 checkpoint inference 비교를 완료하기
전에는 `INFERENCE_VERIFIED`로 승격하지 않는다.
