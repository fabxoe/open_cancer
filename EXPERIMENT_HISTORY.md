...
| EXP-653 | COMPLETED | 2heej | #653 | EXP-527 + 격리된 MMR 유전자(MLH1/MSH2/PMS2, EXP-302 번들 panel에서 분리) observable marker proxy 4개 | 0.4480393463 | 미제출 | INFERENCE_VERIFIED | EXP-527 대비 Macro F1 +0.0011670756(게이트 근소 통과)·Log Loss -0.1934038401(크게 개선)이나 **LGG -0.1304·KIRC -0.1071로 -0.05 붕괴 기준 크게 위반**(KIPAN/KIRC·GBMLGG/LGG 계보 혼동 축, EXP-639·645와 동일 패턴 재현) — `ARCHIVE` | [보고서](reports/exp653_mmr_gene_proxy_exp527/README.md) |
...
| 2026-08-04T09:37:57.985911+00:00 | EXP-370 | Gomin-art | `dbe1e9756b25d348e7e0686f3ef437eb8a982e07` / 태그 없음 | SHA-256 일치 | EXP-370 자체 test 라벨 100%, 확률 allclose·최대 차이 1.72e-07, 제출 SHA-256 일치; EXP-223과의 test 비교는 미측정 | 미실행 | INFERENCE_VERIFIED | [comparison](reproducibility/exp370_exp223_stop_notation_normalization/comparison.json) |
...
- 시작/종료: 2026-08-04T09:09:10.198829+00:00 /
  2026-08-04T09:37:56.157106+00:00

#### 실행과 결과

- EXP-223 설정에 simple stop alternate `*`, `X`, `Ter` 정규화를 이식하고
  최신 공용 runner·feature source에서 재학습했다.
- Fold Macro F1: 0.4152716087 / 0.4148617360 / 0.4084748563 /
  0.4187167281 / 0.4401368382
- OOF Macro F1: 0.4195957914
- Fold 표준편차: 0.0108391780, Accuracy: 0.4100951459,
  Log Loss: 1.8631492853
- Public LB: 미제출
- 재현 상태: `INFERENCE_VERIFIED`

#### 불변성 감사와 제한

- train.csv의 simple stop alternate `X`·`Ter` token은 각각 0건이다. 따라서
  정규화 자체는 train 특징을 변경하지 않는다.
- EXP-223 이후 변경된 공용 runner·feature source에서 재학습했으므로 역사적
  EXP-223과의 OOF 차이를 stop 정규화 효과로 해석하지 않는다.
- EXP-223 test 확률 원본이 없어 EXP-370 대비 변경 행 수·평균 확률 차이·최대
  확률 차이는 모두 미측정이다. test 예측 불변을 주장하지 않는다.
- EXP-370 자체 checkpoint 재추론은 test label 100%, probability allclose,
  submission SHA-256 일치를 통과했다. 이는 EXP-223과의 동일성을 뜻하지 않는다.
...
- Report: `reports/exp370_exp223_stop_notation_normalization/README.md`
- Reproduction: `reproducibility/exp370_exp223_stop_notation_normalization/`
- 결론: 실행·추론 재현성 기록으로 보존하되, 역사적 EXP-223 대비 OOF 차이를
  정규화 효과나 모델 선택 근거로 사용하지 않는다.
...
- 상태: COMPLETED
- 실행자: fabxoe
- Issue/브랜치: #438 / `issue-438-parser-v4-native-semantic-baseline`
- 소스 commit: `055a8b84af9cbed0fa875ef022bdf2d405d7d722`
- 시작/종료: 2026-08-05T04:18:50.331628+00:00 /
  2026-08-05T04:28:35.532212+00:00
- Config: `configs/exp438_parser_v4_native_semantic_baseline.yaml`
- Runner: `scripts/run_exp438_parser_v4_native_semantic_baseline.py`
- OOF Macro F1: 0.4102050373 (L 대비 -0.0030712527, C 대비 -0.0008984094)
- Fold std: 0.0109564971 (L 대비 +0.0014222936)
- Accuracy / Log Loss: 0.4021931946 / 1.9731860161
...
### [EXP-479] Parser v4 native semantic range 3분류 기준선

- 상태: COMPLETED
- 실행자: fabxoe
- Issue/브랜치: #479 / `issue-479-exp-native-semantic-range`
- 소스 commit: `8a6010d6152a1fa8fd4dec795bba866d90137344`
- 시작/종료: 2026-08-05T07:07:09.958226+00:00 /
  2026-08-05T07:19:06.681143+00:00
- Config: `configs/exp479_parser_v4_native_semantic_range.yaml`
- Runner: `scripts/run_exp479_parser_v4_native_semantic_range.py`
- Metrics/Report: `reports/exp479_parser_v4_native_semantic_range/`
- 부모: EXP-469
- 유일한 schema 변경: 기존 다섯 consequence·token-count·gene any·mutation
  presence·모델·fold·seed를 유지하고 `range_stop`, `range_no_change`를 추가했다.
- Fold Macro F1: 0.3984987531, 0.4235728121, 0.4047336390,
  0.3981967162, 0.4190945801
- OOF Macro F1: 0.4087566023 (EXP-469 대비 `-0.0030251756`)
- Fold 표준편차: 0.0105763236 (EXP-469 대비 `+0.0016671733`)
- Accuracy / Log Loss: 0.4005805515 / 1.9471516609
- 최대 클래스 하락/상승: LAML `-0.0383621` / KIRC `+0.0388446`
- Public LB: 미제출
- 재현 상태: `INFERENCE_VERIFIED` — 저장 checkpoint 재추론에서 submission
  SHA-256 byte 일치, test label 100% 일치, 확률 최대 절대 차이 `1.82e-7`.
...
- 상태/실행자: COMPLETED / fabxoe
- Issue/브랜치: #565 / `issue-565-lightgbm-parser-only`
- 소스 commit: `e58e0ac6ab32ff2c25c15b50811b978af24dc824`
- 시작/종료: 2026-08-06T02:15:19.633007+00:00 /
  2026-08-06T02:20:37.026960+00:00
- 부모: EXP-527
- Config/Runner: `configs/exp565_lightgbm_parser_only.yaml` /
  `scripts/run_exp565_lightgbm_parser_only.py`
- 단일 arm: EXP-527의 parser-v4 부모 피처만 유지하고 26개 class-cosine은
  제외. 세 LightGBM arm 공통 설정과 validation Macro F1 checkpoint 사용.
- Fold Macro F1: 0.4129549238, 0.4284829407, 0.4192353662,
  0.4209726006, 0.4548071621
- OOF Macro F1 / fold std: 0.4272525489 / 0.0146212064
- Accuracy / Log Loss: 0.4236413482 / 1.8319022146
- EXP-527 대비: F1 `-0.0196197218`, 라벨 불일치율 `40.0903%`,
  정오답 상관 `0.6196193`
- Public LB: 미제출
- 재현 상태: `INFERENCE_VERIFIED`
...
- 상태/실행자: COMPLETED / 2heej
- Issue/브랜치: #623 / `issue-623-exp527-exp596-fixed-blend`
- Config/Runner: `configs/exp623_exp527_exp596_fixed_blend.yaml` /
  `scripts/run_exp623_exp527_exp596_fixed_blend.py`
- 부모: EXP-527(XGBoost, 26-class 전부 예측하는 최고 안전 단일 모델),
  EXP-596(RandomForest, PR #617에서 diversity-gate PASS 확인:
  CatBoost/LightGBM 대비 correctness_pearson 0.728, label_disagreement 0.309)
- 배경: #505 스태킹 로드맵 S1(사전 고정 단순 평균) 단계. 직전
  EXP-579(EXP-527 XGBoost + EXP-567 LightGBM 0.5/0.5)는 두 boosting 모델의
  오류 상관이 높아 두 부모보다 낮은 점수로 ARCHIVE됐다. 이번 실험은 구조적으로
  다른 오류를 만드는 RandomForest로 같은 S1 고정 블렌드를 재시도해, "모델을
  다양화해도 단순 평균이 개선되지 않는다"는 이전 결론이 다양성이 실제로 확보된
  조합에서도 성립하는지 확인한다.
- 방법: 재학습 없음. `0.5 * EXP-527 확률 + 0.5 * EXP-596 확률` 산술 평균
  (inference-only, EXP-075/135/253/484/579와 동일 패턴). 가중치는 평가 전에
  고정했다. EXP-527의 로컬 OOF/test 확률이 없어(`oof/`·`preds/`는 Git
  비추적) `scripts/run_exp527_parser_v4_class_cosine_loo.py`를 재실행해
  결정론적으로 재생성했고(재현 macro_f1 0.4468722707131544 — History 기록값과
  일치), History 인터페이스 계약(`ID,SUBCLASS_TRUE,SUBCLASS_PRED,FOLD,PROBA_*`)에
  맞춰 최근 공용 러너가 추가한 `SUBCLASS_TRUE_MERGED` 열만 제거한 로컬 사본을
  블렌드 입력으로 사용했다.
- Fold Macro F1: 0.4537973768, 0.4560116314, 0.4835674278, 0.4670821609,
  0.4502940139
- OOF Macro F1 / fold 평균 / fold std: 0.4617833378 / 0.4621505222 /
  0.0120916724
- Accuracy / Log Loss: 0.4450895017 / 1.9898206493
- EXP-527 대비: Macro F1 `+0.0149110671`, fold std `+0.0057123539`,
  Accuracy `+0.0111272375`, Log Loss `-0.0376680592`(개선)
- fold별 비교(EXP-527 → EXP-623): fold0 `+0.0194640726`, fold1
  `+0.0053991767`, fold2 `+0.0348273296`, fold3 `+0.0169832459`, fold4
  `-0.0008716539`. 5개 fold 중 4개가 뚜렷이 개선했고 나머지 1개(fold4)는
  사실상 동일해, fold std 증가는 일부 fold 붕괴가 아니라 fold마다 다른
  개선폭에서 비롯됐다(EXP-484와 같은 패턴).
- 클래스별 비교(EXP-527 대비): 큰 개선 DLBC `+0.1144`, LGG `+0.0525`,
  ACC `+0.0410`, SARC `+0.0295`, BLCA `+0.0297`; 큰 하락 COAD `-0.0312`,
  KIPAN `-0.0253`, SKCM `-0.0200`. `-0.05` 붕괴 기준을 넘는 클래스는 없다.
- 참고자료(비공식, RUN_MODE=explore, 파일 미저장): EXP-527 가중치를
  1.0~0.3 구간에서 0.1 단위로 스윕한 결과 OOF Macro F1이 0.4/0.6에서
  0.4639450817로 가장 높았고 0.5/0.5(0.4617833378)보다 근소하게 나았다.
  이 값은 사후에 같은 OOF로 찾은 것이라 채택 근거로 쓰지 않으며, 별도
  Issue에서 사전 고정 없이는 공식 실행하지 않는다.
...
- Public LB: 미제출
- 재현 상태: `INFERENCE_VERIFIED` — 결정론적 블렌드 재계산으로 제출 SHA-256·
  라벨·확률 일치 확인(최대 절대 오차 `1e-6` 이내)
- 판단: **`ADOPT_WITH_CAUTION`**. OOF Macro F1·Accuracy·Log Loss가 모두
  뚜렷이 개선했고 클래스 붕괴도 없다. fold std만 사전 설정 임계값(`+0.002`)을
  초과했지만 EXP-484와 동일하게 fold 붕괴가 아니라 불균등한 개선폭이 원인임을
  fold별 분해로 확인했다. EXP-579(boosting+boosting)의 실패와 대비해, 실제
  오류 다양성이 확보된 모델(RandomForest)을 블렌드하면 단순 평균도 개선될 수
...
### [EXP-653] EXP-527 격리된 MMR 유전자(MLH1/MSH2/PMS2) proxy — ARCHIVE

- 상태/실행자: COMPLETED / 2heej
- Issue/브랜치: #653 / `issue-653-mmr-gene-proxy-exp527`
- Config/Runner: `configs/exp653_mmr_gene_proxy_exp527.yaml` /
  `scripts/run_exp653_mmr_gene_proxy_exp527.py`
- 부모: EXP-527
- 유일한 변경: EXP-302에서 KRAS/NRAS/BRAF와 한 panel로 묶여 통째로
  기각됐던 MMR 유전자(MLH1/MSH2/MSH6/PMS2)를
  `knowledge/mmr_gene_proxy_v1.json` 단일 panel로 격리한 stateless
  observable marker proxy 4개(`any_mutated`, `any_nonsynonymous`,
  `any_lof`, `multi_gene_mutated`) 추가. 4,384개 실제 패널에는 MSH6가
  없어 MLH1/MSH2/PMS2 3개만 실제 사용됨(전체 train 기준 any_mutated
  183명, any_lof 37명 양성). `ObservableMarkerFamily` 구현은 EXP-302와
  동일, 코드 변경 없음.
- 구현 버그 수정 이력: 최초 실행(source `c7eaf87`)은 semantic-equivalence
  검사를 parent와 이미 병합된 행렬에 대해 parent 자신을 reference로
  호출해 parent feature 대부분이 자기 자신과 중복 판정돼 삭제되는 버그가
  있었다(그 결과 OOF `0.4181`, CESC·PAAD·SKCM 등 무관한 클래스까지 광범위
  붕괴 — History에 기록하지 않고 산출물 삭제). 수정(source `7157700`)은
  병합 전 MMR 후보 4개 열만 별도로 검사하도록 변경했고 단위 테스트로
  검증했다. 아래 결과는 수정판 재실행이다.
- fold-safe semantic equivalence 검사: 중복 열 0개(4개 전부 생존, fold별
  feature 수 88/89→92/93으로 정확히 +4)
- Fold Macro F1: 0.4370363935, 0.4480151366, 0.4546643661, 0.4447492757,
  0.4515918473
- OOF Macro F1 / fold 평균 / fold std: 0.4480393463 / 0.4472114039 /
  0.0060823323
- Accuracy / Log Loss: 0.4318658281 / 1.8340848684
- EXP-527 대비: Macro F1 +0.0011670756(게이트 `≥0.001` 근소 통과), fold
  std -0.0002969862(소폭 개선), Log Loss -0.1934038401(크게 개선)
- 클래스별 최대 하락: **LGG -0.1304(0.4889→0.3585), KIRC
  -0.1071(0.2813→0.1741)** — 사전 고정 -0.05 붕괴 기준을 크게 위반. 다음은
  PAAD -0.0204로 기준 이내. 최대 개선: TGCT +0.0448, BRCA +0.0431, BLCA
  +0.0364
- 해석: KIPAN/KIRC·GBMLGG/LGG 계보 혼동은 이 저장소에서 생물학적으로
  무관해 보이는 feature 추가에서도 반복 재현되는 실패 축이다(EXP-639
  LGG -0.1123/KIRC -0.0512, EXP-645 LGG -0.1235/KIRC -0.0550). Log Loss
  개선이 커도 클래스 붕괴 기준을 사후 완화하지 않는다.
- Test 영향(EXP-527 대비): 2,546개 test 행 전부에서 확률 변화, argmax
  변경 325행(12.8%), 평균 절대 확률 차이 0.0155228842, 확률 상관 0.98389
- Public LB: 미제출
- 재현 상태: `INFERENCE_VERIFIED` — 제출 SHA-256 완전 일치, label
  agreement 1.0, 확률 최대 절대 오차 1.49e-7
- 판단: `ARCHIVE`. Macro F1 gate는 근소 통과했지만 LGG·KIRC 붕괴가 기준을
  크게 초과해 채택하지 않는다. EXP-527을 대체하지 않으며, 이 결과를
  근거로 MMR·observable marker panel 계열 feature를 추가로 탐색하지
  않는다.
